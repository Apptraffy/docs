...
def get_traffy_tasks(resource_id: str, telegram_chat_id: str) -> GetTraffyTasksResponse:
    url = "https://api.traffy.site/v1/mixer/bot/pick_tasks"
    params = {
        "resource_id": resource_id,
        "telegram_id": telegram_chat_id,
        "max_tasks": 3  # This value will be automatically converted to a string
    }

    response = requests.get(url, params=params)
    response.raise_for_status()  # Raise an exception in case of an HTTP error
    return response.json()  # It is assumed that the returned data conforms to GetTraffyTasksResponse

# Example usage:
if __name__ == "__main__":
    resource_id = "your_resource_id"
    telegram_chat_id = "your_telegram_chat_id"  # Pass the chat id as a string or number; it will be converted
...
class CheckTraffyTaskNotCompleted(TypedDict):
    is_completed: bool  # Will be False
    token: None

class CheckTraffyTaskCompleted(TypedDict):
    is_completed: bool  # Will be True
    token: str

CheckTraffyTaskResponse = Union[CheckTraffyTaskNotCompleted, CheckTraffyTaskCompleted]

def check_subscription_traffy(resource_id: str, telegram_chat_id: str, task_id: str) -> CheckTraffyTaskResponse:
    ...
    response = requests.get(url, params=params)
    response.raise_for_status()  # Raise an error if no successful response is received
    return response.json()  # It is assumed that the returned data conforms to CheckTraffyTaskResponse
