...
def get_traffy_tasks(resource_id: str, telegram_chat_id: str) -> GetTraffyTasksResponse:
    ...
    params = {
        ...
        "max_tasks": 3  # Значение будет автоматически преобразовано в строку
    }

    response = requests.get(url, params=params)
    response.raise_for_status()  # выбрасываем исключение в случае HTTP ошибки
    return response.json()  # предполагается, что возвращаемое значение соответствует GetTraffyTasksResponse
...
class CheckTraffyTaskNotCompleted(TypedDict):
    is_completed: bool  # Будет False
    token: None

class CheckTraffyTaskCompleted(TypedDict):
    is_completed: bool  # Будет True
    token: str
...
def check_subscription_traffy(resource_id: str, telegram_chat_id: str, task_id: str) -> CheckTraffyTaskResponse:
    ...
    response = requests.get(url, params=params)
    response.raise_for_status()  # выброс ошибки при отсутствии успешного ответа
    return response.json()  # предполагается, что возвращаемые данные соответствуют CheckTraffyTaskResponse
